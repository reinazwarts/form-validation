let formValidation = formValidation || (function() {
  'use strict';
  ...
  const init = function() {
    ...
    document.addEventListener('keyup', function(event){
      if (!hasClass(event.target, 'had-error')) return;//IE10 support
      validateField(event);
    });
  ...
  const validateField = function(event) {
    ...
    if(event.target.form) {
      ...
      if (!hasClass(event.target.form, settings.cssClassForm)) return;//IE10 support
  ...
  const validateForm = function(event) {
    ...
    if (!hasClass(event.target, settings.cssClassForm)) return;//IE10 support
